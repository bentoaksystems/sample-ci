...
describe('Deny Policy Of Context-Hook', () => {
  let contextHookArr = [
  ...
  let rpJar, userId, contextHooks, roles, policies_context_hook;
  beforeEach(async done => {
    ...
    policies_context_hook = await ContextHookPolicy.model().bulkCreate(ContextHookPolicyArr);
    ...
    try {
      ...
      const res = await rp({
        ...
        body: {
          context: 'Sys',
          is_command: true,
          name: 'denyPolicyContexHook',
          payload: {
            // context_hook_id: contextHooks[0].id,
            // context_hook_policy_id: policies_context_hook[0].id
          }
        },
        jar: rpJar,
        json: true,
      ...
      this.fail('expect error when id is undefined');
      done();
    } catch (err) {
      expect(err.statusCode).toBe(500);
      expect(err.error).toBe('context_hook_id && context_hook_policy_id is required');
      done();
    }
  });
  ...
  it('expect delete context_hook_policy for this context', async function(done) {
    try {
      this.done = done;
      const res = await rp({
        method: 'POST',
      ...
      expect(res.statusCode).toBe(200);
      const contextHookPolicyFound = await ContextHookPolicy.model().find({
        where: { id: policies_context_hook[0].id, context_hook_id: contextHooks[0].id }
      });
      expect(contextHookPolicyFound).toBe(null);
      done();
    } catch (err) {
